...
class StickerMaster(Star):
    ...
    async def send_sticker(self, event: AstrMessageEvent, meaning: str):
        ...
        【使用频率要求】
        - 每隔 2~3 条消息就应该主动发一次，不需要等特别明显的情绪才发
        - 对方说完一句话、结束一个话题、打招呼、道晚安等日常节点都适合发
        - 宁可多发也不要憋着不发，表情包让对话更有温度
        - 以下任何一种情况都可以发：
        ...
        Args:
            meaning(string): 描述当前情绪或场景的词语，如：好想你、开心、无语、生气、思念、嘻嘻等。
        """
        # ── 概率门控 ──────────────────────────────────────────
        prob = float(self.config.get("sticker_probability", 0.4))
        if random.random() > prob:
            logger.debug(f"[StickerMaster] 概率未命中（{prob:.0%}），跳过本次发送")
            return
